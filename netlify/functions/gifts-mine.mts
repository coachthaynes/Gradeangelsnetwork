...
import { donorLabel, newGiftToken } from "../lib/gifts.mts";

// A teacher's gifts: their balance, their gift link, and who has given.
// POST turns the gift link on or off, or saves the note supporters see.
export default async (req: Request) => {
  ...
  const [me] = await db.sql`SELECT gift_balance_cents, gift_link_token, gift_note FROM users WHERE id = ${session.id}`;
